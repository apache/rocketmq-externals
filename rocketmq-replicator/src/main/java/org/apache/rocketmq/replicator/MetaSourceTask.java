...
public class MetaSourceTask extends SourceTask {
    ...
    public void start(KeyValue config) {
        ...
        this.store = new OffsetSyncStore(this.config);
        this.store.start();
        this.started = true;
    }
    ...
    public Collection<SourceDataEntry> poll() {
        ...
        for (String group : groups) {
            ...
            for (Map.Entry<MessageQueue, OffsetWrapper> offsetTable : stats.getOffsetTable().entrySet()) {
                ...
                String targetBrokerName = this.store.getTargetBrokerName(mq.getBrokerName());
                String targetTopic = Utils.getTargetTopic(mq.getTopic(), config.getRenamePattern());

                Schema schema = new Schema();
                schema.setDataSource(this.config.getSourceRocketmq());
                schema.setName(SchemaEnum.OFFSET.name());
                schema.setFields(new ArrayList<>());
                schema.getFields().add(new Field(0, FieldName.OFFSET.getKey(), FieldType.INT64));
                schema.getFields().add(new Field(1, FieldName.BROKER_NAME.getKey(), FieldType.STRING));
                schema.getFields().add(new Field(2, FieldName.TOPIC.getKey(), FieldType.STRING));
                schema.getFields().add(new Field(3, FieldName.QUEUE_ID.getKey(), FieldType.INT64));
                schema.getFields().add(new Field(4, FieldName.CONSUME_GROUP.getKey(), FieldType.STRING));

                DataEntryBuilder dataEntryBuilder = new DataEntryBuilder(schema);
                dataEntryBuilder.timestamp(System.currentTimeMillis()).entryType(EntryType.UPDATE);
                dataEntryBuilder.putFiled(FieldName.OFFSET.getKey(), targetOffset);
                dataEntryBuilder.putFiled(FieldName.BROKER_NAME.getKey(), targetBrokerName);
                dataEntryBuilder.putFiled(FieldName.TOPIC.getKey(), targetTopic);
                dataEntryBuilder.putFiled(FieldName.QUEUE_ID.getKey(), mq.getQueueId());
                dataEntryBuilder.putFiled(FieldName.CONSUME_GROUP.getKey(), group);
                SourceDataEntry sourceDataEntry = dataEntryBuilder.buildSourceDataEntry(null, null);
