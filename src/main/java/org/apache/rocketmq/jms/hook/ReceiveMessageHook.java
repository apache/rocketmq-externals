...
public class ReceiveMessageHook {
    ...
    private void validate(Message message) throws JMSException {
        if (message.getJMSExpiration() != 0 && System.currentTimeMillis() > message.getJMSExpiration()) {
            throw new MessageExpiredException(String.format("This message[id=%s] has been expired", message.getJMSMessageID()));
        }
    }

    public void setProviderProperties(Message message) throws JMSException {
        //JMSXRcvTimestamp
        message.setLongProperty(JMSPropertiesEnum.JMSXRcvTimestamp.name(), System.currentTimeMillis());
    }
}
